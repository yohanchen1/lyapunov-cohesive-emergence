"""
DCNG publication-quality figures — ICML/NeurIPS style.
Ocean Dusk palette, serif fonts, 300dpi, clean spines.
Reads data from dgng_final2.py, regenerates all figures.
"""
import numpy as np
...
# ── Publication styling — readable at single-column (~8.5cm) ──
plt.rcParams.update({
...
# Ocean Dusk palette
C_TEAL, C_GREEN, C_GOLD, C_ORANGE, C_CORAL = "#264653", "#2A9D8F", "#E9C46A", "#F4A261", "#E76F51"
C_BLUE, C_RED, C_GRAY = "#2878B5", "#C82423", "#9E9E9E"
FIG_FULL = (6.75, 5.5)   # ICML full-width
FIG_HALF = (3.25, 2.6)   # ICML single-column
...
def energy(x,w_vec,edges,alpha):
    px=phi(x); e=sum(w_vec[k]*px[i]*px[j] for k,(i,j) in enumerate(edges))
    return -e + np.sum(G_phi(x)) + 0.5*alpha*np.sum(w_vec**2)
...
def analyze(x,w_vec,edges,alpha):
    ...
    for idx,(i,j) in enumerate(edges):
        wv=w_vec[idx]
        if (px[i]>1e-6 and px[j]>1e-6) or (px[i]<-1e-6 and px[j]<-1e-6): intra.append(wv)
        elif (px[i]>1e-6 and px[j]<-1e-6) or (px[i]<-1e-6 and px[j]>1e-6): inter.append(wv)
    r = {'Vp':len(Vp),'Vn':len(Vn),'V0':len(V0),'Vp_idx':Vp,'Vn_idx':Vn,'V0_idx':V0}
    if intra and inter:
        r['theta_high']=min(intra); r['theta_low']=max(inter)
        r['delta']=r['theta_high']-r['theta_low']; r['cohesive']=r['delta']>0
    return r
...
def make_er(n,p_edge): return np.array([(i,j) for i in range(n) for j in range(i+1,n) if rng.random()<p_edge],dtype=int)
def make_ba(n,m_param): return np.array(list(nx.barabasi_albert_graph(n,m_param,seed=int(rng.integers(0,2**31))).edges()),dtype=int)
def make_ws(n,k,p_rewire): return np.array(list(nx.watts_strogatz_graph(n,k,p_rewire,seed=int(rng.integers(0,2**31))).edges()),dtype=int)

# ── Generate publication figure for one config ──
def pub_figure(T,X,W,edges_used,E_hist,ca,eps,alpha,name,topo_label):
    ...
    k_val = 6.0 / np.sqrt(n) if n > 20 else 4.0 / max(np.sqrt(n),1)
    ...
    base = max(5, 80.0 / (n**0.4))
    sizes=[base*2.2 if i in Vp or i in Vn else base*0.25 for i in range(n)]
    edge_colors=[C_GREEN if (i in Vp and j in Vp) or (i in Vn and j in Vn)
                 ...
                 else '#E8E8E8' for i,j in edges_used]
    ew=0.12 if n>50 else 0.25
    nx.draw_networkx_edges(G,pos,edge_color=edge_colors,width=ew,alpha=0.3,ax=ax)
    nx.draw_networkx_nodes(G,pos,node_color=node_colors,node_size=sizes,linewidths=0.05,ax=ax)
    ax.set_title(f'{topo_label} (={n}$, $\delta={d_:.2f}$)',fontsize=12,fontweight='bold')
    ax.axis('off')

    # (1,1) Weight matrix heatmap
    ...
    ax.set_title('Weight matrix ^*$',fontsize=12,fontweight='bold')
    ...
    if 'theta_high' in ca:
        ...
        ax.axvline(th,color=C_GREEN,ls=':',lw=2)
        ax.axvline(tl,color=C_RED,ls=':',lw=2)
        ax.text(0.98,0.92,f'$	heta_h={th:.1f}$
$	heta_l={tl:.1f}$',
    ...
    # (2,1)+(2,2) Self-consistency check
    ...
    if pv:
        ...
        ax.scatter(range(len(pv)),pv,c=C_BLUE,s=14,alpha=0.5,marker='x',label='$lpha w_{ij}/\phi(x_j)$ pred')
...
def check_cohesive(eps,x0_fixed,w0_fixed,n,edges,alpha,T_max):
    ...
    Vp=np.sum(px_f>1e-6); Vn=np.sum(px_f<-1e-6)
    intra,inter=[],[]
    for idx,(i,j) in enumerate(edges):
        wv=w_f[idx]
        if (px_f[i]>1e-6 and px_f[j]>1e-6) or (px_f[i]<-1e-6 and px_f[j]<-1e-6): intra.append(wv)
        elif (px_f[i]>1e-6 and px_f[j]<-1e-6) or (px_f[i]<-1e-6 and px_f[j]>1e-6): inter.append(wv)
    d_=0.0
...
def pub_phase_transition():
    ...
    lo,hi=0.10,0.16
    for _ in range(20):
        mid=(lo+hi)/2; _,_,d_=check_cohesive(mid,x0_fixed,w0_fixed,n,edges,alpha,T_max)
        if d_>0: hi=mid
        else: lo=mid
    eps_c=(lo+hi)/2

    # Fine sweep
    eps_vals=np.linspace(0,0.35,250)
    deltas=[]; vp_list=[]; vn_list=[]
    for eps in eps_vals:
        vp,vn,d_=check_cohesive(eps,x0_fixed,w0_fixed,n,edges,alpha,T_max)
        deltas.append(d_); vp_list.append(vp); vn_list.append(vn)
    deltas=np.array(deltas); vp_list=np.array(vp_list); vn_list=np.array(vn_list)
    ...
    ax1.set_xlabel('Learning rate $\\varepsilon$',fontsize=10)
    ax1.set_ylabel('Cohesive separation $\\delta$',fontsize=10)
    ax1.set_title('Phase transition: $\\delta$ vs $\\varepsilon$',fontsize=10,fontweight='bold')
    ax1.legend(fontsize=8,loc='lower right')
    ax1.set_xlim(0,0.35); ax1.set_ylim(-0.5,7.5)
    ax1.annotate('Trivial\nequilibrium',xy=(0.06,0.5),fontsize=8,color=C_GRAY,ha='center')
    ax1.annotate(f'Cohesive\n$\\delta\\approx 2/\\alpha$',xy=(0.22,6.2),fontsize=8,color=C_GREEN,ha='center')
    ...
    ax2.set_title(f'Critical region ($\\varepsilon_c={eps_c:.4f}$)',fontsize=10,fontweight='bold')
    ax2.legend(fontsize=8,loc='lower right')

    fig.tight_layout()
    fig.savefig(f'{OUT_DIR}/dgng_pub_phase_transition.png',dpi=300,facecolor='white')
    plt.close(fig)

# ── Topology comparison summary figure ──
def pub_topology_summary():
    ...
    for n,p_er in [(10,0.25),(20,0.12),(50,0.05),(100,0.03)]:
        for topo_name,make_fn,args in [('ER',make_er,(p_er,)),('BA',make_ba,(3,)),('WS',make_ws,(6,0.1))]:
            try: edges=make_fn(n,*args)
            except: continue
            res=run_one(n,edges,0.5,0.3,800 if n<50 else 1500,seed=42)
            if res:
                T,X,W,_,E_hist,ca=res
                results.append((topo_name,n,len(edges),ca['Vp'],ca['Vn'],ca.get('delta',0),
                               ca['n_inc']==0,ca['dx_max']))

    fig,axes=plt.subplots(1,3,figsize=(6.75,2.5))

    # Panel A: delta vs n by topology
    ax=axes[0]
    for topo,color,marker in [('ER',C_TEAL,'o'),('BA',C_CORAL,'s'),('WS',C_GOLD,'^')]:
        pts=[(n,d_) for t,n,m,vp,vn,d_,ok,dx in results if t==topo]
        if pts:
            ns,ds=zip(*sorted(pts))
            ax.plot(ns,ds,color=color,marker=marker,lw=1.5,markersize=6,label=topo,markerfacecolor=color)
    ax.axhline(y=2/0.3,color=C_GRAY,ls=':',lw=1,label='$2/\\alpha=6.67$')
    ax.set_xlabel('$n$',fontsize=10); ax.set_ylabel('$\\delta$',fontsize=10)
    ax.set_title('Cohesive separation',fontsize=9,fontweight='bold')
    ax.legend(fontsize=8,loc='lower right'); ax.set_ylim(0,8)

    # Panel B: V0 ratio
    ax=axes[1]
    for topo,color,marker in [('ER',C_TEAL,'o'),('BA',C_CORAL,'s'),('WS',C_GOLD,'^')]:
        pts=[(n,v0/n*100) for t,n,m,vp,vn,d_,ok,dx in results if t==topo
             for v0 in [(n-vp-vn)]]
        if pts:
            ns,v0r=zip(*sorted(pts))
            ax.plot(ns,v0r,color=color,marker=marker,lw=1.5,markersize=6,label=topo,markerfacecolor=color)
    ax.set_xlabel('$n$',fontsize=10); ax.set_ylabel('$V^0$ ratio (%)',fontsize=10)
    ax.set_title('Uncommitted nodes',fontsize=9,fontweight='bold')
    ax.legend(fontsize=8,loc='upper right')

    # Panel C: Convergence error
    ax=axes[2]
    for topo,color,marker in [('ER',C_TEAL,'o'),('BA',C_CORAL,'s'),('WS',C_GOLD,'^')]:
        pts=[(n,dx) for t,n,m,vp,vn,d_,ok,dx in results if t==topo]
        if pts:
            ns,dxs=zip(*sorted(pts))
            ax.semilogy(ns,dxs,color=color,marker=marker,lw=1.5,markersize=6,label=topo,markerfacecolor=color)
    ax.set_xlabel('$n$',fontsize=10); ax.set_ylabel('$\\max\\|\\dot x\\|(T)$',fontsize=10)
    ax.set_title('Convergence error',fontsize=9,fontweight='bold')
    ax.legend(fontsize=8,loc='lower right')

    for ax in axes: ax.grid(alpha=0.12,lw=0.4)
    fig.tight_layout()
    fig.savefig(f'{OUT_DIR}/dgng_pub_topology_summary.png',dpi=300,facecolor='white')
    plt.close(fig)
...
if __name__=='__main__':
    ...
    for name,n,edges,eps,alpha,T_max,topo_label in configs:
        ...
        if res:
            ...
            pub_figure(T,X,W,edges_used,E_hist,ca,eps,alpha,name,topo_label)
